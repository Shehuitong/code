...
public interface UserFavoritesMapper extends BaseMapper<UserFavorites> {
    ...
    @Select("SELECT * FROM UserFavorites WHERE user_id = #{userId} AND target_type = #{targetType} ORDER BY create_time DESC")
    // 根据用户ID和目标类型查询收藏记录
    ...
    @Select("select * from UserFavorites where user_id = #{userId} and target_id = #{targetId} and target_type = #{targetType} and favorite_status = #{status} limit 1")
    ...
    @Delete("delete from UserFavorites where user_id = #{userId} and target_id = #{targetId} and target_type = #{targetType}")
    ...
    @Select("SELECT COUNT(DISTINCT user_id) FROM UserFavorites " +
